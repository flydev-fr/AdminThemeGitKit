...
var ProcessWireAdminTheme = {
	...
	ready: function() {
		...
		var $body = $("body"),
				$main = $("#main"),
				$masthead = $('#pw-masthead'),
				$pwHeight = $('#pw-masthead').data('pw-height'),
		...
		// masthead
		// $('#NotificationBug').click(this.bugClickEvent);
		this.setupMasthead();
		// headroom.js
		$bug = $('#NotificationBug');
		$("[data-headroom]").headroom({
			...
			onPin: function () {
				/*if($bug.hasClass('open')) {
					$('#NotificationMenu').css('margin-top', $pwHeight);
				}
				else {
					$('#NotificationMenu').css('margin-top', 0);
				}*/
				$notifications.css({
					'top': $pwHeight,
					'position': 'fixed'
			...
			onUnpin: function () {
				...
				$notifications.css({ 'top': 0, 'position': 'fixed' }); 
			},
		});
		// tabs
		this.setupWireTabs();
		...
		$mainHeight = parseInt($masthead.data('pw-height')) * 1.5;
		console.log($mainHeight);
		$main.attr('style', 'margin-top:' + $mainHeight + 'px !important');
		$($notifications).css({
			'top': $pwHeight,
			'position': 'fixed'
		});
